...
# However, we can override the magic method __add__ in a class to allow for
# this functionality.
...
# later on, so we need to ensure that the typecasting will be possible. Use the
# built-in function isinstance to check if the value passed in is a str and the
# built-in str.isnumeric() method to ensure that all characters in the string
# value are numeric. If the value is a string, but has non-numeric characters,
# raise an Exception with the message "String value can have only numeric
...
# thing_2. This method should cast both self.val and thing_2 as ints and return
# their sum.
